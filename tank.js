...
function fetchDeviceStatusHPM806(printer) {
	...
	$.get("http://izumi.bobst.nyu.edu/print/fetchUrl.php?rnd="+Math.random(),{url:urlDeviceStatus,printer:printer,rnd:Math.random()}, function(data) { //if the printer responds to the HTTP request, this function will run
...
function fetchDeviceStatusHP9050(printer) {
	...
	//pass the url of the printer device status page and the printer object
	//	also another random number for the same reason
	$.post("http://izumi.bobst.nyu.edu/print/fetchUrl.php?rnd="+Math.random(),{url:urlDeviceStatus,printer:printer,rnd:Math.random()}, function(data) { //if the printer responds to the HTTP request, this function will run
	//data is the HTML generated by the device status page by way of fetchUrl.php
		var strHTML = data;
		var statusValue = 0;
...
function fetchDeviceStatusHP4700(printer) {
	...
	//data is the HTML generated by the device status page by way of fetchUrl.php
		...
		for (var i=0; i<trays.length; i++) {
			...
			scp printers.js kcp288@access.cims.nyu.edu:/web/kcp288/
			printerBox.append(divTray);
		}
