...
function fetchDeviceStatusHPM806(printer) {
	...
	$.get("fetchUrl.php?rnd="+Math.random(),{url:urlDeviceStatus,printer:printer,rnd:Math.random()}, function(data) { //if the printer responds to the HTTP request, this function will run
...
function fetchDeviceStatusHP9050(printer) {
	...
	console.log(urlDeviceStatus);
	var invocation = new XMLHttpRequest();
   
	function callOtherDomain() {
  		if(invocation) {    
   		 invocation.open('GET', urlDeviceStatus, true);
   		 invocation.onreadystatechange = handler;
   		 invocation.send(); 
  		}
	}

	$.get(urlDeviceStatus,function(data){
		console.log("??? " + data);
	});
	...
	//pass the url of the printer device status page and the printer object
	//	also another random number for the same reason
	$.post("http://izumi.bobst.nyu.edu/fetchUrl.php?rnd="+Math.random(),{url:urlDeviceStatus,printer:printer,rnd:Math.random()}, function(data) { //if the printer responds to the HTTP request, this function will run
	//data is the HTML generated by the device status page by way of fetchUrl.php
		var strHTML = data;
		var statusValue = 0;
